...
const addressSchema = z.object({
  street: z.string().min(2, "Street/Area is required").max(200),
...
const DELIVERY_FEE_DEFAULT = 0; // Admin sets delivery cost after order
...
function buildWhatsAppMessage(params: {
  ...
  let msg = `🛒 *NEW ORDER FROM SOS HARDWARE APP — ${orderNumber}*\n`;
  msg += `━━━━━━━━━━━━━━━━━━\n`;
...
function validateOrderNumbers(items: { quantity: number; price: number; name?: string }[], totalAmount: number) {
  const invalidQuantityItem = items.find((item) => !Number.isFinite(item.quantity) || item.quantity < 1 || item.quantity > MAX_ORDER_QUANTITY);
  if (invalidQuantityItem) {
    throw new Error(`Quantity is too large. Please keep each item below ${MAX_ORDER_QUANTITY.toLocaleString()}.`);
  }

  const invalidLineTotalItem = items.find((item) => {
    const lineTotal = item.price * item.quantity;
    return !Number.isFinite(lineTotal) || lineTotal > MAX_ORDER_AMOUNT;
  });

  if (invalidLineTotalItem) {
    const maxAllowedForItem = Math.max(1, Math.floor(MAX_ORDER_AMOUNT / invalidLineTotalItem.price));
    throw new Error(
      `${invalidLineTotalItem.name || "This product"} exceeds the order limit. Please keep its quantity at ${maxAllowedForItem.toLocaleString()} or less.`
    );
  }

  if (!Number.isFinite(totalAmount) || totalAmount > MAX_ORDER_AMOUNT) {
    throw new Error(`Order total is too large. Please reduce the quantity so the total stays below ${formatPrice(MAX_ORDER_AMOUNT)}.`);
  }
}
...
export default function Checkout() {
  ...
  const [submittedItems, setSubmittedItems] = useState<WhatsAppOrderItem[]>([]);
  const [submittedSubtotal, setSubmittedSubtotal] = useState(0);
  const [submittedTotalAmount, setSubmittedTotalAmount] = useState(0);
  const [submittedDeliveryType, setSubmittedDeliveryType] = useState<"pickup" | "delivery">("pickup");
  const [submittedAddress, setSubmittedAddress] = useState<AddressData | null>(null);

  const subtotal = total();
  const deliveryFee = 0; // Admin will set delivery cost after order is placed
  ...
  const sendToWhatsApp = (
    ...
    orderAddressData?: AddressData | null
  ) => {
    if (!contactData) return;

    const resolvedItems = orderItems ?? submittedItems;
    const resolvedSubtotal = orderSubtotal ?? submittedSubtotal;
    const resolvedTotalAmount = orderTotalAmount ?? submittedTotalAmount;
    const resolvedDeliveryType = orderDeliveryType ?? submittedDeliveryType;
    const resolvedAddress = orderAddressData ?? submittedAddress;
  ...
  const handlePayLater = async () => {
    ...
    try {
      ...
      const orderItemsForWhatsApp = items.map((i) => ({
        name: i.product.name,
        quantity: i.quantity,
      ...
      try {
        validateOrderNumbers(orderItemsForWhatsApp, totalAmount);
      } catch (validationError: any) {
        setSubmittedItems(orderItemsForWhatsApp);
        setSubmittedSubtotal(subtotal);
        setSubmittedTotalAmount(totalAmount);
        setSubmittedDeliveryType(deliveryType);
        setSubmittedAddress(addressData);
        setOrderNumber(orderNum);
        setPaymentMethod("pay_later");
        sendToWhatsApp(orderNum, false, null, orderItemsForWhatsApp, subtotal, totalAmount, deliveryType, addressData);
        setPaymentState("success");
        clearCart();
        toast({
          title: "Order sent to WhatsApp",
          description: `${validationError.message} The order was sent on WhatsApp instead of being saved in the database.`,
        });
        return;
      }
      ...
      <div className="min-h-screen flex items-center justify-center py-20">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center max-w-md mx-auto px-4 space-y-6">
          ...
          <div className="glass-card p-5 text-left space-y-3 text-sm border border-border/50">
            ...
            <p className="text-muted-foreground">3️⃣ Confirm {formatPrice(totalAmount)} to SOS Hardware & Glassmart</p>
          </div>
                ...
                <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                  ...
                  <form onSubmit={contactForm.handleSubmit(handleContactSubmit)} className="space-y-4">
                    ...
                    <div>
                      ...
                      <Input id="email" type="email" placeholder="john@example.com" {...contactForm.register("email")} className="mt-1" />
                    </div>
                ...
                <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-6">
                    ...
                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setDeliveryType("delivery")} className={`glass-card p-5 text-left transition-all ${deliveryType === "delivery" ? "border-primary ring-2 ring-primary/30 shadow-lg shadow-primary/10" : "hover:border-primary/30"}`}>
                      ...
                      <p className="text-xs text-primary font-bold mt-1">Cost set by admin</p>
                      <p className="text-xs text-muted-foreground mt-1">Admin determines fee</p>
                    </motion.button>
                  </div>
